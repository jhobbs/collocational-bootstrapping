...
This experiment implements [REPRODUCTION_PLAN.md](REPRODUCTION_PLAN.md), the
original `reproduce_english_childes_with_pylangacq.md` guide.
...
fitting scripts. Published figures, tables, fit results and provenance records
live under `output/`; [PUBLISHED_ARTIFACTS.json](PUBLISHED_ARTIFACTS.json) lists
their checksums and the bulk generated inputs retained locally.
The commands below use existing parsed/fitted data, so they do not download or
reparse the corpora. The plotting command uses the published summaries; the
audits also require the local utterance and pair CSVs generated by the earlier
commands. Use new output paths if these destinations already exist.
...
Raw transcripts, bulk generated utterance/pair inputs and environments are
ignored by Git. Selected analysis outputs are published according to
[PUBLISHED_ARTIFACTS.json](PUBLISHED_ARTIFACTS.json). Use a new output directory
for each run; the CLIs protect existing results. Bootstrap, fixed-verb
